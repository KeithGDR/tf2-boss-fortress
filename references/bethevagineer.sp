...
#define SOUND_JUMP1                     "saxton_hale/vagineer_responce_jump_1.wav"
#define SOUND_JUMP2                     "saxton_hale/vagineer_responce_jump_2.wav"
 
#define SOUND_KILLSCOUT         "saxton_hale/lolwut_0.wav"
#define SOUND_KILLPYRO          "saxton_hale/lolwut_4.wav"
#define SOUND_KILLDEMO          "saxton_hale/lolwut_1.wav"
#define SOUND_KILLHEAVY         "saxton_hale/vagineer_responce_taunt_2.wav"
#define SOUND_KILLMEDIC         "saxton_hale/vagineer_responce_taunt_1.wav"
#define SOUND_KILLSNIPER1       "saxton_hale/vagineer_responce_taunt_3.wav"
#define SOUND_KILLSPY1          "saxton_hale/vagineer_responce_taunt_4.wav"
#define SOUND_KILLENGY1         "saxton_hale/vagineer_responce_taunt_5.wav"
#define SOUND_KILLSENTRY        "saxton_hale/vagineer_responce_intro.wav"
 
#define SOUND_KILL              "saxton_hale/lolwut_5.wav"
 
#define SOUND_TAUNT1            "saxton_hale/vagineer_responce_rage_1.wav"
#define SOUND_TAUNT2            "saxton_hale/vagineer_responce_rage_2.wav"
#define SOUND_TAUNT3            "saxton_hale/lolwut_2.wav"
#define SOUND_TAUNT4            "saxton_hale/lolwut_3.wav"
 
#define MODEL_HALE                      "models/player/saxton_hale/vagineer_v134.mdl"
...
new bool:g_bVagineer                            [MAXPLAYERS + 1];
 
// ====[ PLUGIN ]==============================================================
public Plugin:myinfo =
{
        name = "Be The Vagineer",
        author = "Rose Rossi & juanjpro",
        description = "Be the Vagineer!",
...
{
        CreateConVar("sm_bethevagineer_version", PLUGIN_VERSION, "Be The Vagineer Version", FCVAR_PLUGIN | FCVAR_SPONLY | FCVAR_DONTRECORD | FCVAR_NOTIFY);
 
        g_hCvarEnabled = CreateConVar("sm_bethevagineer_enabled", "1", "Enable Be The Vagineer\n0 = Disabled\n1 = Enabled", _, true, 0.0, true, 1.0);
        g_bEnabled = GetConVarBool(g_hCvarEnabled);
        HookConVarChange(g_hCvarEnabled, OnConVarChange);
 
        g_hCvarHealth = CreateConVar("sm_bethevagineer_health", "10000", "Health Vagineer gets", _, true, 325.0);
        g_iHealth = GetConVarInt(g_hCvarHealth);
        HookConVarChange(g_hCvarHealth, OnConVarChange);
 
        g_hCvarModel = CreateConVar("sm_bethevagineer_model", "1", "Enable setting player model\n0 = Disabled\n1 = Enabled", _, true, 0.0, true, 1.0);
        g_bModel = GetConVarBool(g_hCvarModel);
        HookConVarChange(g_hCvarModel, OnConVarChange);
 
        g_hCvarSuperJumps = CreateConVar("sm_bethevagineer_superjumps", "1", "Enable super jumps\n0 = Disabled\n1 = Enabled", _, true, 0.0, true, 1.0);
        g_bSuperJumps = GetConVarBool(g_hCvarSuperJumps);
        HookConVarChange(g_hCvarSuperJumps, OnConVarChange);
 
        g_hCvarRaging = CreateConVar("sm_bethevagineer_raging", "2", "Rage percent regeneration\n0 = Disabled", _, true, 0.0);
        g_iRaging = GetConVarInt(g_hCvarRaging);
        HookConVarChange(g_hCvarRaging, OnConVarChange);
 
        g_hCvarSounds = CreateConVar("sm_bethevagineer_sounds", "1", "Enable vagineer sounds\n0 = Disabled\n1 = Enabled", _, true, 0.0, true, 1.0);
        g_bSounds = GetConVarBool(g_hCvarSounds);
        HookConVarChange(g_hCvarSounds, OnConVarChange);
 
        g_hCvarRedLimit = CreateConVar("sm_bethevagineer_redlimit", "-1", "Max red players allowed to be vagineer\n-1 = Disabled", _, true, -1.0);
        g_iRedLimit = GetConVarInt(g_hCvarRedLimit);
        HookConVarChange(g_hCvarRedLimit, OnConVarChange);
 
        g_hCvarBluLimit = CreateConVar("sm_bethevagineer_blulimit", "-1", "Max blu players allowed to be vagineer\n-1 = Disabled", _, true, -1.0);
        g_iBluLimit = GetConVarInt(g_hCvarBluLimit);
        HookConVarChange(g_hCvarBluLimit, OnConVarChange);
 
        g_hCvarRespawn = CreateConVar("sm_bethevagineer_respawn", "1", "Enable respawning as vagineer\n0 = Disabled\n1 = Enabled", _, true, -1.0);
        ...
        AutoExecConfig(true, "plugin.bethevagineer");
 
        RegAdminCmd("sm_vagineer", BeTheVagineerCmd, ADMFLAG_GENERIC, "Be The Vagineer");
        ...
        LoadTranslations("bethevagineer.phrases");
...
public Action:OnPlayerSpawn(Handle:hEvent, const String:strName[], bool:bDontBroadcast)
{
        ...
        if(!g_bVagineer[iClient] || !g_bEnabled || !g_bModel || !g_bRespawn)
        {
                ...
                CreateTimer(0.5, Timer_MakeVagineer, GetClientUserId(iClient), TIMER_FLAG_NO_MAPCHANGE);
...
public Action:OnPlayerDeath(Handle:hEvent, const String:strName[], bool:bDontBroadcast)
{
        ...
        if(!IsValidClient(iAttacker) || !IsPlayerAlive(iAttacker) || !g_bVagineer[iAttacker])
                return Plugin_Continue;
...
public Action:SoundHook(iClients[64], &iClientCount, String:strSample[PLATFORM_MAX_PATH], &iEntity, &iChannel, &Float:fVolume, &iLevel, &iPitch, &iFlags)
{
        ...
        if(g_bVagineer[iEntity] && StrContains(strSample, "saxton_hale", false) == -1)
                return Plugin_Stop;
        return Plugin_Continue;
}
 
// ====[ COMMANDS ]============================================================
public Action:BeTheVagineerCmd(iClient, iArgs)
{
        if(!g_bEnabled)
                return Plugin_Continue;
        ...
        if(iArgs != 0 && iArgs != 2)
        {
                ReplyToCommand(iClient, "[SM] Usage: sm_vagineer <target> <0/1>");
        ...
        if(iArgs == 0)
        {
                ...
                {
                        SetGlobalTransTarget(iClient);
                        PrintToChat(iClient, "[SM] %t", "vagineer_disabled");
                        g_bVagineer[iClient] = false;
                ...
                else
                {
                        ...
                        SetGlobalTransTarget(iClient);
                        PrintToChat(iClient, "[SM] %t", "vagineer_enabled");
                        g_iOldClass[iClient] = TF2_GetPlayerClass(iClient);
                        g_bVagineer[iClient] = true;
 
                        if(IsPlayerAlive(iClient))
                                MakePlayerVagineer(iClient);
                }
        }
        else if(iArgs == 2)
        {
                if(!CheckCommandAccess(iClient, "sm_vagineer_target", ADMFLAG_GENERIC))
                {
                ...
                if(iValue == 0 && g_bVagineer[iTarget])
                {
                        SetGlobalTransTarget(iTarget);
                        PrintToChat(iTarget, "[SM] %t", "vagineer_disabled");
                        g_bVagineer[iTarget] = false;
                ...
                else if(iValue == 1 && !g_bVagineer[iTarget])
                {
                        ...
                        SetGlobalTransTarget(iTarget);
                        PrintToChat(iTarget, "[SM] %t", "vagineer_enabled");
                        g_iOldClass[iTarget] = TF2_GetPlayerClass(iTarget);
 
                        if(IsPlayerAlive(iTarget))
                                MakePlayerVagineer(iTarget);
                }
        }
        return Plugin_Handled;
}
...
public Action:TauntCmd(iClient, const String:strCommand[], iArgc)
{
        ...
        if(!IsValidClient(iClient) || !IsPlayerAlive(iClient) || !g_bVagineer[iClient] || g_iRage[iClient] < 100.0)
                return Plugin_Continue;
...
public Action:Timer_SuperJump(Handle:hTimer)
{
        ...
        {
                if(!IsPlayerAlive(i) || !g_bVagineer[i])
                        continue;
...
public Action:Timer_MakeVagineer(Handle:hTimer, any:iClientId)
{
        new iClient = GetClientOfUserId(iClientId);
...
stock GetTeamHaleCount(iTeam)
{
        ...
        for(new i = 1; i <= MaxClients; i++) if(IsValidClient(i))
        {
                if(g_bVagineer[i] && GetClientTeam(i) == iTeam)
                        iCount++;
        }
        return iCount;
}
 
stock MakePlayerVagineer(iClient)
{
        g_bVagineer[iClient] = true;
        ...
        Format(strAttributes, sizeof(strAttributes), "150 ; 1.0 ; 436; 1.0; 402 ; 1.0 ; 68 ; 2.0 ; 2 ; 3.1 ; 259 ; 1.0 ; 252 ; 0.6 ; 275 ; 1 ; 26 ; %i ; 107 ; 1.7 ; 214 ; %d", g_iHealth - 200, GetRandomInt(9999, 99999));
        TF2Items_GiveWeapon(iClient, "tf_weapon_wrench", 7, 100, 4, strAttributes);
